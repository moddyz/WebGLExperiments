...
function main() {
  ...
  function render(currentTime) {
    ...
    // Update scene state.
    sceneState.cubeRotation += sceneState.deltaTime;

    // Draw the scene
    drawScene(gl, programInfo, buffers, texture, sceneState);
...
/// Loads a texture downloaded from an `url` into the `gl` context.
function loadTexture(gl, url) {
  // Set `texture` as the target of further texture operations.
...
//
// Draw the scene, using the `gl` context.
//
function drawScene(gl, programInfo, buffers, texture, sceneState) {
  ...
  mat4.rotate(
      ...
      sceneState.cubeRotation,  // amount to rotate in radians
      [0, 0, 1]);
  ...
  mat4.rotate(
      modelViewMatrix,                // destination matrix
      modelViewMatrix,                // matrix to rotate
      sceneState.cubeRotation * 0.5,  // amount to rotate in radians
      [0, 1, 0]);
  ...
  // Tell webgl how to pull out the texture coordinates from buffer
  {
    ...
    gl.vertexAttribPointer(
        programInfo.attribLocations.textureCoord, numComponents, type,
        normalize, stride, offset);
